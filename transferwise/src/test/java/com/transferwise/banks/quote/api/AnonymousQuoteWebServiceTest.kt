...
internal class AnonymousQuoteWebServiceTest {

    @Test
    internal fun `get quote from anonymous endpoint`() = runBlockingTest {
        ...
        whenever(webService.getAnonymousQuote(fakeRequest)).thenReturn(mockQuoteResponse)

        val quote = AnonymousQuoteWebService(webService).getQuote(5, fakeRequest)

        assertThat(quote).isEqualTo(mockQuoteResponse)
    }
}
